...
public class CsarProcessor {
	...
	/**
	 * This funcion marks an artifact sign flag based on policy file If the artifact
	 * cannot be found in csar. It sets warning message in response object. Warning
	 * is also set if policy file mentions an artifact in META-INF folder or TOSCA
	 * meta folder.
	 * 
	 * @param csar
	 * @param policyInfo
	...
	public void setSignFlagOfArtifacts(Csar csar, PolicyInfo policyInfo) {
		...
		for (Artifact artifact : csar.getArtifacts()) {
			if (policyInfo.isSignAllArtifacts() || policyInfo.getArtifactsToSign().contains(artifact.getName()))
				artifact.setToSign(true);
		}
	}

	/**
	 * This funcion marks an artifact encryption flag based on policy file If the
	 * artifact cannot be found in csar. It sets warning message in response object.
	 * Warning is also set if policy file mentions an artifact in META-INF folder or
	 * TOSCA meta folder.
	 * 
	 * @param csar
	 * @param policyInfo
	...
	public void setEncFlagOfArtifacts(Csar csar, PolicyInfo policyInfo) {
		for (Artifact artifact : csar.getArtifacts()) {
			if (policyInfo.isEncryptAllArtifacts() || policyInfo.getArtifactsToEncrypt().contains(artifact.getName()))
	...
	/**
	 * This function creates java.util.jar.Manifest object from byte[] contents of
	 * manifest in csar. It removes all entries in manifest which do not have
	 * corrosponding file in csar. And if a file exist and entry does not, then this
	 * function adds this new entry in manifest file.
	 * 
	...
	public Manifest createManifest(byte[] manifestContent, ArrayList<Artifact> artifacts) throws Exception {
		...
		try {
			...
			// We have to do this because java Manifest only works with Manifest-Version
			// name
			// This should be replaced by Constant.MENIFEST_FILE_MAIN_VERSION_ENTRY at the
			// time of writing
			SecurityProcessor securityProcessor = new SecurityProcessor();
			securityProcessor.updateManifestMainKey(manifest, Constant.MENIFEST_FILE_MAIN_VERSION_ENTRY,
					Constant.MENIFEST_FILE_WOKRING_MAIN_VERSION_ENTRY);
			...
			// adding new entries in manifest (if does not exist already against actual file
			// in csar)
			for (Artifact artifact : artifacts) {
	...
	/**
	 * This function convert a manifest into an array of bytes We can also specify
	 * if we want to update manifest main key name
	 * 
	 * @param manifest
	 ...
	 * @throws Exception
	 */
	public byte[] serializeManifest(Manifest manifest, boolean replaceMainEntry, String oldKeyName, String newKeyName)
			throws Exception {
	...
	public byte[] generateCsar(Csar csar) throws Exception {
		...
		try {
			...
			zipArhchiveOs.putArchiveEntry(entry);
			zipArhchiveOs.write(new CsarProcessor().serializeManifest(csar.getManifest(), true, "Manifest-Version",
					Constant.MENIFEST_FILE_MAIN_VERSION_ENTRY));
			zipArhchiveOs.closeArchiveEntry();
			...
			// closing (not only flushing) to make all the bytes flushed to byte array
			// (otherwise unexpected EOF when extracting)
			zipArhchiveOs.close();
			bos.close();
